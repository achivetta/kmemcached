                               ...
                               char **tokens, int ntokens)
{
  ...
  delta = simple_strtoull(tokens[2], NULL, 10);
                          ...
                          char **tokens, int ntokens)
{
  ...
  if (ntokens == 2)
  {
    timeout= (uint32_t)simple_strtoul(tokens[1], NULL, 10);
  }
                                     ...
                                     char **end, ssize_t length)
{
  ...
  uint64_t cas, result_cas;
  unsigned long nbytes, need;
  void *data;
  protocol_binary_response_status rval;
  ...
  flags= (uint32_t)simple_strtoul(tokens[2], NULL, 10);
  timeout= (uint32_t)simple_strtoul(tokens[3], NULL, 10);
  nbytes= simple_strtoul(tokens[4], NULL, 10);

  /* Do we have all data? */
  need= nbytes + (unsigned long)((*end - start) + 1) + 2; /* \n\r\n */
  ...
  case ADD_CMD:
    ...
    break;
  case CAS_CMD:
    cas= simple_strtoull(tokens[5], NULL, 10);
    /* FALLTHROUGH */
  case REPLACE_CMD:
  ...
  *endptr= ptr;
  ...
  do {
    ...
    if (client->ascii_command == GET_CMD ||
       ...
       */
      char *tokens[10];
      int error, ntokens= ascii_tokenize_command(ptr, end, tokens, 10);
