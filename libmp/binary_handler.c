                     ...
                     protocol_binary_response_header *response)
{
  ...
  len = sizeof(*response) + htonl(response->response.bodylen);
  offset= 0;
  ptr= (void*)response;
...
get_response_handler(const void *cookie,
  ...
  {
    keylen= 0;
    response.message.header.response.keylen= htons(keylen);
...
memcached_protocol_event_t memcached_binary_protocol_process_data(memcached_protocol_client_st *client, ssize_t *length, void **endptr)
{
  ...
  protocol_binary_request_header *header= (void*)client->root->input_buffer;
  ssize_t len= *length;
         ...
         (len >= (ssize_t)(sizeof(*header) + ntohl(header->request.bodylen))))
  {
    ...
    rv= execute_command(client, header);
    ...
    total= (ssize_t)(sizeof(*header) + ntohl(header->request.bodylen));
    len -= total;
    if (len > 0)
    {
